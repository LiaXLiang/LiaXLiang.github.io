...
#### 
Sorting the array simplifies the process of eliminating duplicates and enables the use of the two-pointer technique efficiently. Once the array is sorted, we can fix one element (say a) and use a two-pointer approach to find b and c such that 𝑎 + 𝑏 + 𝑐 = 0. Sorting also ensures that duplicate values are adjacent, making it easier to skip repeated elements and avoid generating duplicate triplets.
...
After finding a valid triplet, we want to move both pointers inward to continue searching for other valid pairs. However, we must avoid adding the same triplet multiple times due to duplicate values in the array.
...
### Special Considerations
#### **Scenario 1: Skipping Duplicate *a* Values**
Skip duplicate *a* values in the outer loop to prevent redundant triplets (e.g., multiple -10s as a leading to repeated results like before).
...
class Solution {
    ...
    public void quickSort(int[] nums, int left, int right){
        //错误：应该是left >= right, 而不是left == right
        if(left >= right) return;
    ...
    private static int partition(int[] nums, int left, int right){
        ...
        while(i <= j){
            //注意这里是两个while,而不是两个if, 
            //注意两个while里都要添加判断条件i <= j &&
